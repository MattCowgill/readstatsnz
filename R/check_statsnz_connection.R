#' This function checks to see if the StatsNZ website is available.
...
check_statsnz_connection <- function() {
  snz_url_works <- url_exists("https://www.stats.govt.nz")


  if (isFALSE(snz_url_works)) {
    snz_url_works_nocurl <- url_exists_nocurl("https://www.stats.govt.nz")

    if (isFALSE(snz_url_works_nocurl)) {
      stop(
        "R cannot access the StatsNZ website.",
...
url_exists <- function(url = "https://www.stats.govt.nz") {
...
#' 200 range; `FALSE` otherwise.
#' @noRd
url_exists_nocurl <- function(url = "https://www.stats.govt.nz") {
  con <- url(url)
  out <- suppressWarnings(tryCatch(readLines(con), error = function(e) e))
  snz_url_works <- all(class(out) != "error")
  close(con)
  return(snz_url_works)
}
